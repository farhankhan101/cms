...
// Fetch Sent Parcels (matched by NIC)
$sent_parcels = [];
if ($user_nic) {
...
<div class="card" style="margin-bottom: 30px; border-top: 4px solid var(--primary);">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <div>
            <h2 style="font-weight: 800; color: var(--primary-dark);">Sent Shipments</h2>
            <p style="color: var(--text-med); font-size: 14px;">Showing all parcels sent using NIC: <strong><?php echo sanitize($user_nic); ?></strong></p>
        </div>
        <div style="background: var(--bg-light); padding: 10px 20px; border-radius: 12px; font-weight: 700;">
            Total: <?php echo count($sent_parcels); ?>
        </div>
    </div>

    <?php if (empty($sent_parcels)): ?>
        <div style="text-align: center; padding: 60px;">
            <i class="fas fa-paper-plane" style="font-size: 50px; color: #eee; margin-bottom: 20px;"></i>
    ...
    <?php else: ?>
        <div class="table-responsive">
            <table>
                ...
                <tbody>
                    <?php foreach ($sent_parcels as $p): ?>
                        <tr>
                            <td style="font-weight: 700; color: var(--primary);">#<?php echo sanitize($p['tracking_no']); ?></td>
                            <td><?php echo sanitize($p['receiver_name']); ?></td>
                            <td><?php echo sanitize($p['to_city']); ?></td>
